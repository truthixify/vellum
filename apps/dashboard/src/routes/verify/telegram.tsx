...
import {
  confirmTelegramClaims,
  readTelegramAccountClaims,
  type TelegramAccountClaim,
  type TelegramClaimConfirmation,
} from "@/lib/telegram-claim-reader";
import {
  TelegramVerificationRequestError,
  telegramSubmissionFromSearch,
  telegramVerificationErrorMessage,
  parseTelegramVerificationSearch,
  requestTelegramAuthorization,
  type TelegramSubmission,
  type TelegramVerificationErrorCode,
...
export const Route = createFileRoute("/verify/telegram")({
  validateSearch: parseTelegramVerificationSearch,
...
function telegramAccountLabel(account: { display_name: string; username?: string }): string {
  return account.username ? `@${account.username}` : account.display_name;
}

function telegramRoleLabel(role: "administrator" | "member" | "owner"): string {
  return role === "owner" ? "Owner" : role === "administrator" ? "Administrator" : "Member";
}

function TelegramVerificationPage() {
  useDocumentTitle("Telegram verification");
  const queryClient = useQueryClient();
  const search = Route.useSearch();
  const submission = telegramSubmissionFromSearch(search);
  ...
  const [reportedConfirmation, setReportedConfirmation] = useState<{
    ...
    result?: TelegramClaimConfirmation;
  }>();
  const confirmation =
    ...
    <div className="account-verification-page">
      ...
      <header className="account-verification-header">
        <div>
          ...
          <p>
            Verify a Telegram account and current membership in recognized CKB communities for a
...
function VerificationSteps({
  ...
  confirmation: TelegramClaimConfirmation | null | undefined;
...
function ConnectionPanel({
  ...
  callbackError?: TelegramVerificationErrorCode;
  retryAt?: number;
  ...
  const identities = useQuery({
    queryKey: ["telegram-verification-dids", lock?.codeHash, lock?.hashType, lock?.args],
    queryFn: async () => {
  ...
  const existingClaims = useQuery({
    queryKey: ["telegram-account-claims", selectedDid, issuer.data?.did],
    queryFn: () => readTelegramAccountClaims(TESTNET_CLIENT, selectedDid, issuer.data!),
  ...
  async function startVerification() {
    ...
    setStarting(true);
    try {
      if (!signer) throw new TelegramVerificationRequestError("subject_control_invalid");
      const authorizationUrl = await requestTelegramAuthorization(selectedDid, signer);
      window.location.assign(authorizationUrl);
    } catch (error) {
      setStartError(
        error instanceof TelegramVerificationRequestError
          ? error.message
          : "Telegram verification could not be started.",
      );
      setStarting(false);
    }
  }
  ...
  return (
    <section aria-labelledby="telegram-connect-title">
      ...
      {(callbackError || startError) && (
        <div className="account-verification-alert" role="alert">
          ...
          <div>
            ...
            <p>{startError ?? telegramVerificationErrorMessage(callbackError)}</p>
            {retryDate && (
              <p>
                Try again after{" "}
      ...
      {!signer ? (
        ...
        <div className="account-verification-form">
          {records.length > 1 ? (
            <>
              <label htmlFor="telegram-subject">Target identity</label>
              <select
                id="telegram-subject"
                value={selectedDid}
                onChange={(event) => {
          ...
          {issuer.isError || existingClaims.isError ? (
            ...
            >
              <AlertCircle size={18} aria-hidden="true" />
              <div>
                <strong>Telegram claim status unavailable</strong>
                <p>Vellum could not check this identity's active Telegram claims.</p>
                <button
                  className="v-button v-button--quiet"
                  type="button"
                  onClick={() =>
            ...
            <div className="account-verification-claim-loading" role="status" aria-live="polite">
              <span className="pulse-dot" aria-hidden="true" />
              Checking existing Telegram claims for {shorten(selectedDid)}
            </div>
    ...
    <div className="account-verification-existing">
      <div className="account-verification-existing__heading">
        ...
        <div>
          ...
          <p>
            {claims.length === 1
              ? "This DID has an active Telegram identity claim."
              : `This DID has ${claims.length} active Telegram identity claims.`}
          </p>
        </div>
      </div>
            ...
            {claim.community ? (
              <div className="account-verification-community-list">
                {claim.community.community.memberships.map((membership) => (
                  <div key={membership.chat_id}>
                    <span>
                      <Users size={13} aria-hidden="true" /> {membership.community_name}
                    </span>
                    <small>
                      {telegramRoleLabel(membership.member_role)} · {membership.community_type}
                    </small>
                  </div>
                ))}
...
function SubmittedVerification({
  ...
  const confirmation = useQuery({
    ...
    queryFn: () => confirmTelegramClaims(TESTNET_CLIENT, submission, issuer.data!),
    enabled: !!issuer.data,
    retry: false,
    ...
    <section aria-labelledby="telegram-result-title">
      ...
      <dl className="account-verification-result-grid">
        ...
        <div>
          ...
          <dd className="mono">vellum.social.telegram.v1</dd>
        </div>
        <div>
        ...
        {submission.communityClaimId ? (
          <div>
            <dt>Community schema</dt>
            <dd className="mono">vellum.community.telegram.v1</dd>
          </div>
        ) : null}
        <div>
    ...
    return (
      <div className="account-verification-alert" role="alert">
        ...
        <div>
          ...
          <p>No live Telegram Claim Cells were created.</p>
        </div>
      </div>
    );
  }
...
function IncompleteResult() {
  return (
    <section aria-labelledby="telegram-incomplete-title">
      <div className="account-verification-alert" role="alert">
        <AlertCircle size={18} aria-hidden="true" />
        <div>
          <strong id="telegram-incomplete-title">Incomplete callback result</strong>
          <p>The public claim references were missing or invalid. Start a new verification.</p>
          <Link className="v-button v-button--quiet" to="/verify/telegram" search={{}}>
            Start again
          </Link>
        </div>
      </div>
...
function ProtocolSummary({ submission }: { submission?: TelegramSubmission }) {
  return (
    <aside className="account-verification-summary" aria-label="Verification details">
      ...
      <dl>
        <div>
          <dt>Account check</dt>
          <dd>Telegram OpenID Connect</dd>
        </div>
        <div>
          <dt>Identity claim</dt>
          <dd>User ID, display name, optional public username, verification time</dd>
        </div>
        <div>
          <dt>Community claim</dt>
          <dd>Current configured chat membership and role; valid for 30 days</dd>
        </div>
        <div>
          <dt>Not collected</dt>

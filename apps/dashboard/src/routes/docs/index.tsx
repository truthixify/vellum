...
const CARDS: Array<{
...
}> = [
  {
    ...
    body: "The identifier algorithm (type-id style BLAKE2b truncated to 20 bytes), the document shape compatible with did:plc, and the create/update/deactivate lifecycle as enforced by the on-chain Type Script.",
  },
  {
    to: "/docs/cell-model",
    number: "02",
    title: "CKB Cell model",
    body: "A primer on the unit of state your DID lives in. Capacity (storage rent), Lock Script (who controls it), Type Script (state-transition rules), and what Live vs Dead means.",
  },
  {
    to: "/docs/resolution",
    number: "03",
    title: "Resolution",
    body: "How a did:ckb identifier becomes a document. Indexer queries by type-script args, molecule + DAG-CBOR decoding, conflict resolution when more than one cell claims an identifier, and the reverse-lookup path from a CKB address.",
  },
  {
    to: "/docs/migration",
    number: "04",
    title: "did:plc migration",
    body: "How an existing AT Protocol identity lands on CKB without losing its history. The 72-hour finalisation window, the rotation-key contest path, and what the local_id field is doing.",
  },
];
  ...
  return (
    <>
      ...
      <p className="text-base leading-[1.6] text-ink mb-3">
        ...
        implemented in the upstream <span className="font-mono">did-ckb</span> repository. These
        pages explain the method in the level of detail a holder, an integrator, or a reviewer would
        need.
      </p>
      <p className="text-base leading-[1.6] text-ink mb-12">
        Start with the method spec, then read about the Cell model if you have not worked with CKB
        before. The resolution page covers how documents are read off the chain, and the migration
        page covers bringing an existing <span className="font-mono">did:plc</span> identity over.
      </p>

...
describe("verification failure logging", () => {
  test("records useful context without exposing 32-byte secrets", () => {
    const output = spyOn(console, "error").mockImplementation(() => undefined);
    try {
      logVerificationFailure({
        error: new Error(`submission failed for 0x${"ab".repeat(32)}\nnext line`),
        platform: "discord",
        requestId: "request-1",
        stage: "issuance",
      });
      ...
      expect(JSON.parse(String(output.mock.calls[0][0]))).toEqual({
        ...
        error: "submission failed for [redacted] next line",
        errorName: "Error",
        platform: "discord",
        requestId: "request-1",
        stage: "issuance",
